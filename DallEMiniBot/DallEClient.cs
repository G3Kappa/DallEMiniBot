...
public sealed class DallEClient
{
    ...
    public async Task<IEnumerable<byte[]>?> TryGetImages(string query)
    {
        var jsonQuery = JsonConvert.SerializeObject(new { prompt = query });
        var response = await HttpClient.PostAsync("/generate", new StringContent(jsonQuery, Encoding.UTF8, "application/json"));
        try
        {
